...
- 텍스트 분석
  - 토픽모델링: 문서가 가지고 있는 여러 가지 토픽을 찾아주는 기법
  - 감성분석: 문장에서 주관적인 감성을 나타내는 정보를 찾아내어 성향을 분석하는 것
  - 텍스트 분류: 텍스르를 지정한 카테고리로 분류하는 기법
  - 텍스트 군집화: 벡터 연산을 통해 단어 벡터 간 유사도를 이용하여 단어의 관계를 파악하고 비슷한 의미를 가지는 단어들끼리 군집 형성

p.354 사회연결망 분석
- SNA Social Network Analysis
  - 개인과 집단들 간의 관계를 노드와 링크로 모델링하여 그것의 위상구조와 확산 및 진화 과정을 계량적으로 분석하는 방법론
  - 사회연결망에서 개인 또는 집단이 하나의 노드이며, 노드 사이에 존재하는 연결은 선으로 표현됨
- SNA 분류
  - 집합론적 방법: 객체들의 집합에서 각 객체들 간의 관계를 쌍으로 표현
  - 그래프 이론을 이용한 방법: 객체를 점을 표현하고, 두 객체 간의 연결은 두 점을 연결하는 선으로 표현
  - 행렬을 이용한 방법

P.356 사회연결망 분석에서 네트워크 구조를 파악하기 위한 기법
- 중심성
  - 연결정도 중심성
  - 근접 중심성
  - 매개 중심성
